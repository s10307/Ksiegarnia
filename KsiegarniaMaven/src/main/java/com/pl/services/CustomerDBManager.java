...
public class CustomerDBManager {
	...
	private PreparedStatement findCustomerBySurnameStmt;
	...
	public CustomerDBManager() {
		try {
			...
			if (!CustomerTableExists) {
				stmt.executeUpdate("CREATE TABLE customer(id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,"
			...
			findCustomerBySurnameStmt = conn
					.prepareStatement("SELECT id FROM customer WHERE surname = ?");
	...
	public List<Integer> findCustomerBySurname(String surname) {
		try {
			...
			ResultSet rs = findCustomerBySurnameStmt.executeQuery();
			while (rs.next())
				result.add(rs.getInt("ID"));
			return result;
		} catch (SQLException e) {
